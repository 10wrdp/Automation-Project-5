...
const issueDescription = faker.lorem.words({min: 1, max: 7}) 
...
describe('Issue create', () => {
  ...
  it('Custom Issue Creation', () => {
    ...
    cy.get('[data-testid="modal:issue-create"]').within(() => {
      ...
      cy.get('[class="sc-iqzUVk cUBVJX"]').contains('Highest').click()
  ...
  it('Random Data Plugin Issue Creation', () => {
    ...
    cy.get('[data-testid="modal:issue-create"]').within(() => {
      ...
      cy.get('[data-testid="select:type"]').click();
      cy.get('[class="sc-bdVaJa dctDAB sc-kjoXOD bTEsIL"]').get('[data-testid="icon:task"]')
          .trigger('click');
      ...
      //Select priority
      cy.get('[data-testid="select:priority"]').click()
      cy.get('[class="sc-qrIAp bkxtXT"]').contains('Low').click()
